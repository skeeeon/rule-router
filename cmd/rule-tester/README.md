...
├── batch-rule.yaml
└── batch-rule_test/
    ├── _test_config.json              # Pre-populated mock trigger
    ├── match_1.json                   # Array input (3 items, 2 match filter)
    ├── match_1_output.json            # Expected: 2 actions (array format)
    ├── not_match_1.json               # Array with no matching items
    └── not_match_2_empty_array.json   # Empty array edge case
```

**Console Output for ForEach Rules:**
```
✓ Detected forEach operation on field: notifications
  Generating array-based test examples...
✓ Scaffolded test directory at: batch-rule_test/

💡 TIP: Your rule uses forEach - example test files include:
   • Array input messages
   • Array output validation (multiple actions)
...
**What Gets Validated:**
- ✅ Action count: 2 (E2 filtered out)
- ✅ Subject templating: `{eventId}` from array element
- ✅ Payload templating: `{@msg.deviceId}` from root message
- ✅ JSON structure of each payload
...
- ✅ Message that matches all conditions
- ✅ Message that fails each condition individually
- ✅ Edge cases (null values, missing fields)
- ✅ Template variable resolution

For **forEach rules**, additionally test:
- ✅ Multiple matching elements (2-3 items)
- ✅ Single matching element
- ✅ No matching elements (all filtered)
- ✅ Empty array
- ✅ Mixed array (some match, some don't)
- ✅ Non-object elements (should skip gracefully)
