...
func setupTestKVContext(t *testing.T) (*KVContext, *LocalKVCache) {
	...
	store3.Put(context.Background(), "is_enabled", []byte(`true`))
	store3.Put(context.Background(), "allowed_ips", []byte(`["10.0.0.1", "10.0.0.2"]`))
...
func TestNewKVContext(t *testing.T) {
	...
	t.Run("initializes successfully", func(t *testing.T) {
		...
		if len(kvCtx.stores) != 3 {
			t.Errorf("Expected 3 KV stores, got %d", len(kvCtx.stores))
		}
...
func TestParseKVField(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid simple path", "@kv.device_config.sensor-123:threshold", "device_config", "sensor-123", []string{"threshold"}, false},
		...
		{"invalid bucket/key format with colon", "@kv.device_config:threshold", "", "", nil, true},
		{"invalid bucket/key format no colon", "@kv.device_config", "", "", nil, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bucket, key, path, err := kvCtx.parseKVField(tt.field)
			if (err != nil) != tt.wantErr {
				t.Fatalf("parseKVField() error = %v, wantErr %v", err, tt.wantErr)
			}
			if !tt.wantErr {
...
func TestGetField_PrimitivesAndArrays(t *testing.T) {
	...
	t.Run("get whole string primitive", func(t *testing.T) {
		...
		if strVal, isStr := val.(string); !isStr || strVal != "sk-abc123" {
			t.Errorf("got %v (%T), want 'sk-abc123' (string)", val, val)
		}
	})
	...
	t.Run("get whole number primitive", func(t *testing.T) {
		...
		if numVal, isNum := val.(float64); !isNum || numVal != 1000 {
			t.Errorf("got %v (%T), want 1000 (float64)", val, val)
		}
	})
	...
	t.Run("get whole boolean primitive", func(t *testing.T) {
		...
		if boolVal, isBool := val.(bool); !isBool || !boolVal {
			t.Errorf("got %v (%T), want true (bool)", val, val)
		}
	})
	...
	t.Run("get array element by index", func(t *testing.T) {
		...
		if strVal, isStr := val.(string); !isStr || strVal != "10.0.0.1" {
			t.Errorf("got %v (%T), want '10.0.0.1' (string)", val, val)
		}
	})
