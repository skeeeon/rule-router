//file: cmd/rule-scheduler/main.go
...
func run() error {
	...
	createApp := func() (lifecycle.Application, error) {
		...
		// Create the scheduler application with the common base.
		return app.NewSchedulerApp(baseApp, cfg)
	}
...
func parseFlags() (*config.Config, string) {
	// Define flags
	configPath := flag.String("config", "config/rule-scheduler.yaml", "path to config file (YAML or JSON)")
	...
	// Load configuration using the Viper-powered loader
