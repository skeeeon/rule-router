...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 go build -o nats-auth-manager ./cmd/nats-auth-manager

FROM alpine:latest
RUN apk --no-cache add ca-certificates
COPY --from=builder /app/nats-auth-manager /usr/local/bin/
ENTRYPOINT ["nats-auth-manager"]
CMD ["-config", "/etc/auth-manager/config.yaml"]
```
...
kind: Deployment
metadata:
  name: nats-auth-manager
spec:
  replicas: 1  # Only need one instance
  selector:
    matchLabels:
      app: nats-auth-manager
  template:
    metadata:
      labels:
        app: nats-auth-manager
    spec:
      containers:
      - name: auth-manager
        image: your-registry/nats-auth-manager:latest
        args: ["-config", "/etc/auth-manager/config.yaml"]
        env:
        ...
        - name: STRIPE_CLIENT_SECRET
          valueFrom:
            secretKeyRef:
              name: api-credentials
              key: stripe-client-secret
        volumeMounts:
        - name: config
          mountPath: /etc/auth-manager
        - name: nats-creds
          mountPath: /etc/nats
      volumes:
      - name: config
        configMap:
          name: auth-manager-config
      - name: nats-creds
        secret:
