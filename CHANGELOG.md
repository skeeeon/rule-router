...
### Improvements
- **Startup warning for schedule rules that publish to unstreamed subjects** — the scheduler now checks each schedule rule's static NATS action subject against the discovered streams at startup (and on KV rule rebuild) and warns once, listing the offenders, when a JetStream-mode action targets a subject no stream captures. Previously this surfaced only as a recurring `timeout waiting for publish ack` per fire, with nothing pointing at the cause; almost always the rule meant `mode: core`. Templated subjects are skipped — they are known only per evaluation.
