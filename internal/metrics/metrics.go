...
func NewMetrics(registry *prometheus.Registry) (*Metrics, error) {
	m := &Metrics{
		...
		forEachIterationsTotal: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				...
				Help: "Total number of forEach array elements processed",
			},
			[]string{"rule_file"},
		),
		forEachFilteredCount: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "foreach_filtered_total",
				Help: "Total number of forEach array elements filtered out",
			},
			[]string{"rule_file"},
		),
		forEachActionsGenerated: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "foreach_actions_generated_total",
				Help: "Total number of actions generated by forEach operations",
			},
			[]string{"rule_file"},
		),
		forEachElementErrors: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Name: "foreach_element_errors_total",
				Help: "Total number of forEach element processing errors",
			},
			[]string{"rule_file", "error_type"},
		),
		forEachDuration: prometheus.NewHistogramVec(
			prometheus.HistogramOpts{
				Name:    "foreach_duration_seconds",
				Help:    "Duration of forEach processing operations",
		...
		arrayOperatorEvaluations: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				...
				Help: "Total number of array operator evaluations (any/all/none)",
			},
	...
	collectors := []prometheus.Collector{
		...
		m.kvCacheMisses,
		m.kvCacheSize,
		m.forEachIterationsTotal,
		m.forEachFilteredCount,
...
func (m *Metrics) IncForEachFiltered(ruleFile string, count int) {
	m.forEachFilteredCount.WithLabelValues(ruleFile).Add(float64(count))
}

func (m *Metrics) IncForEachActionsGenerated(ruleFile string, count int) {
	m.forEachActionsGenerated.WithLabelValues(ruleFile).Add(float64(count))
}

func (m *Metrics) IncForEachElementErrors(ruleFile, errorType string) {
	m.forEachElementErrors.WithLabelValues(ruleFile, errorType).Inc()
}
